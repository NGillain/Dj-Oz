...
local Mix Interprete Projet CWD in
   ...
   CWD = {Property.condGet 'testcwd' 'C://Users//Nathan//Documents//GitHub//Dj-Oz//joie.dj.oz//'}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      fun {ToNote Note} % Transforme les notes en record Note(nom:  octave:  alteration: )
	 case Note
	 ...
	 [] Atom then
      ...
      fun{NoteToEchantillon Note} % renvoie une liste de record echantillon sur base d'une liste de note
	 case Note
	 ...
	 [] note(nom:X octave:Y alteration:Z) then echantillon(hauteur:{ComputeDemiTons Note} duree:1.0 instrument:none)
      ...
      fun{Bourdon X Y} % transformation qui ramene toutes les notes d'un partition a une hauteur definie
	 ...
	 []H|T then 
      ...
      fun{Muet X} % transformation qui annule toutes les hauteurs et retourne un long silence qui dure autant que la partition
	 {Bourdon silence X}
      end
      
      fun{Etirer X Y} % etirer une note ou une partition et l'allonge d'un facteur determine
	 case Y of nil then nil
	 []H|T then
	 ...
	 []H|T then {DureePart T Acc+H.duree}
      ...
      fun{Duree X Y} % transformation qui permet de modifier la duree totale de la partition sans modifier son rythme
	 case Y of nil then nil
	 []H|T then {Etirer X/{DureePart Y 0.0} Y}
	 end
      end
      
      fun{Transpose X Y} % augmente toutes les notes d'un partition d'une hauteur determinee
	 case Y of nil then nil
	 []H|T then
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      fun{ToVector Ech} % Transforme un echatillon en vecteur
	 local
	 ...
	 of H|T then
      ...
      fun{MergeToVector P} % rassemble les vecteurs audio pour ne jouer qu'un seul morceau compose de plusieurs morceaux pondere par une coefficient
	 local
	    M={ListToVector P}
	    fun{Somme X Acc}
	    case X of nil then Acc
	 ...
	 in
      ...
      fun{AddVector V1 V2} % additionne deux vecteurs de tailles différentes
	 ...
	 local
	    ...
	    fun{ReAffect X Y}
	       ...
	       [] H|T then
	          case H of A#B then A/Y#B |{ReAffect T Y} end
	       end
	    end
	 in
	    {ReAffect X Y}
